...
const router = Router();
import { collNames } from "../libs/global.js";
import { set, Types } from "mongoose";
import modelFactory from "../libs/model_factory.js";

set("strictQuery", true);

const GuiaModel = modelFactory("guiaObj", "guias");

router.get("/get/:nroGuiaRem", (req, res) => {
  const match = () => ({
    $match: {
      nroGuiaRem: Types.Decimal128(req.params.nroGuiaRem),
    },
  });
  ...
  GuiaModel.aggregate([
    match(),
    lookup(collNames.destinatarios, "destinatario", "nroDest", "_Destinatario"),
    lookup(collNames.vehiculos, "vehiculo", "nroVehi", "_Vehiculo"),
    lookup(collNames.conductores, "conductor", "nroCondu", "_Conductor"),
    lookup(collNames.comprobantes, "comprobante", "nroComprob", "_Comprobante"),
    lookup(collNames.motivos, "motivo", "nroMotivo", "_Motivo"),
    lookup(
      collNames.transportistas,
      "transportista",
      "nroTransp",
      "_Transportista"
    ),
    lookup(collNames.productos, "detalles.nroProd", "nroProd", "_Productos"),
    project(),
    unset(),
  ])
    .then(
      ...
      (aggrOutput) => {
        const guia = {
          ...
          detalles: aggrOutput[0].detalles.map((detalle) => ({
            cantidad: detalle.cantPedida,
            descripcion: aggrOutput[0]._Productos.find(
              (prod) => prod.nroProd === detalle.nroProd
            ).descr,
...
router.patch("/conforme/:nroGuiaRem", (req, res) => {
  GuiaModel.updateOne(
    { nroGuiaRem: req.params.nroGuiaRem },
